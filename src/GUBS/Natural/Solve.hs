...
defaultProcessor :: (Ord v, Ord f, PP.Pretty f, PP.Pretty v) => Solver -> Processor f Integer v IO
...
solveWithLog :: (Eq f, Eq v, Monad m) => ConstraintSystem f v -> Processor f c v m -> m (Answer f v c, ExecutionLog)
