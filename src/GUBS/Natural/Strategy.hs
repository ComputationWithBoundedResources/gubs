...
import           GUBS.Strategy                 as M hiding (Processor, ProcT)
...
type Processor f c v m = P.Processor f (MaxPoly Var c) (ConstraintSystem f v) m
...
logInterpretation :: (Ord f, Eq c, SemiRing c, PP.Pretty f, PP.Pretty c, Monad m) =>  ConstraintSystem f v -> ProcT f c m ()
...
logConstraint :: (Ord f, Pretty f, Pretty v, Integral c, Monad m) => Interpretation f c -> Constraint (Term f v) -> ProcT f c m ()
...
logConstraints' :: (Ord f, Integral c, Pretty f, Pretty v, Monad m) => ConstraintSystem f v -> ProcT f c m ()
...
logOpenConstraints :: (Ord f, SemiRing c, Max c, IsNat c, Integral c, Pretty f, Pretty v, Monad m) => ConstraintSystem f v -> ProcT f c m ()
